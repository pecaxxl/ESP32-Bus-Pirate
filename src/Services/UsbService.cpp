...
// Implementation file: UsbService.cpp
...
void UsbService::storageBegin(uint8_t cs, uint8_t clk, uint8_t miso, uint8_t mosi) {
...
void UsbService::keyboardSendString(const std::string& text) {
    if (!keyboardActive) return;
...
void UsbService::keyboardSendChunkedString(const std::string& data, size_t chunkSize, unsigned long delayBetweenChunks) {
    if (!keyboardActive) return;
...
void UsbService::mouseMove(int x, int y) {
    // Wait HID init
    while (millis() - hidInitTime < 1500) {
        delay(10);
    }
...
void UsbService::mouseClick(int button) {
    // Wait HID init
    while (millis() - hidInitTime < 1500) {
        delay(10);
    }
...
void UsbService::mouseRelease(int button) {
    if (!mouseActive) return;
    mouse.release(button);
}

void UsbService::gamepadBegin() {
...
void UsbService::gamepadPress(const std::string& name) {
    if (!gamepadActive) return;
...
bool UsbService::isKeyboardActive() const {
    return keyboardActive;
}

bool UsbService::isStorageActive() const {
    return storageActive;
}

bool UsbService::isMouseActive() const {
    return mouseActive;
}


int32_t UsbService::storageReadCallback(uint32_t lba, uint32_t offset, void* buffer, uint32_t bufsize) {
...
int32_t UsbService::storageWriteCallback(uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize) {
    const uint32_t secSize = SD.sectorSize();
    if (secSize == 0) return -1;
...
bool UsbService::usbStartStopCallback(uint8_t power_condition, bool start, bool load_eject) {
    return true;
}

void UsbService::setupStorageEvent() {
